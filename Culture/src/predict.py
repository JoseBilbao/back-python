...
print(retentions_X_train)
retentions_y_train = X_train['ret']
reformations_y_train = X_train['innov']

olsmod = smf.ols(formula='ret ~ acquired + tenure + title_status + male + rating + branch_id + axl', data=retentions_X_train)
olsres = olsmod.fit()

olsmod_innov = smf.ols(formula='innov ~ acquired + tenure + title_status + male + rating + branch_id + axl', data=reformations_X_train)
